  ...
  <div class="container-fluid">
    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body">
		...
		<input type="hidden" name="config_product_upc_hide" value="0" />
		<input type="hidden" name="config_product_ean_hide" value="0" />
		<input type="hidden" name="config_product_jan_hide" value="0" />
		<input type="hidden" name="config_product_isbn_hide" value="0" />
		<input type="hidden" name="config_product_mpn_hide" value="0" />
		<input type="hidden" name="config_review_status" value="0" />
		<input type="hidden" name="config_review_guest" value="0" />
		<input type="hidden" name="config_review_mail" value="0" />
		...
		<input type="hidden" name="config_tax_default" value="shipping" />
		<input type="hidden" name="config_tax_customer" value="shipping" />
		<input type="hidden" name="config_product_count" value="0" />   
		<input type="hidden" name="config_product_upc_hide" value="0" />
		<input type="hidden" name="config_product_ean_hide" value="0" />
		<input type="hidden" name="config_product_jan_hide" value="0" />
		...
		<input type="hidden" name="config_tax_default" value="shipping" />
		<input type="hidden" name="config_tax_customer" value="shipping" />
		<input type="hidden" name="config_tax" value="1" />
		<input type="hidden" name="config_customer_online" value="0" />
		<input type="hidden" name="config_customer_price" value="0" />
		<input type="hidden" name="config_login_attempts" value="<?php echo $config_login_attempts; ?>" />
		<input type="hidden" name="config_account_mail" value="0" />
		<input type="hidden" name="config_invoice_prefix" value="<?php echo $config_invoice_prefix; ?>" />
		<input type="hidden" name="config_cart_weight" value="0" /> 
		<input type="hidden" name="config_checkout_guest" value="0" />
		<input type="hidden" name="config_order_mail" value="0" />
		...
		<input type="hidden" name="config_affiliate_commission" value="<?php echo $config_affiliate_commission; ?>" />
		<input type="hidden" name="config_affiliate_mail" value="0" />	 

		<?php foreach ($currencies as $currency) { ?>
		<input type="hidden" name="config_currency" value="<?php echo $currency['code']; ?>" />
		<?php } ?>

		<?php foreach ($length_classes as $length_class) { ?>
		<input type="hidden" name="config_length_class_id" value="<?php echo $length_class['length_class_id']; ?>" />
		<?php } ?>

		<?php foreach ($weight_classes as $weight_class) { ?>
		<input type="hidden" name="config_weight_class_id" value="<?php echo $weight_class['weight_class_id']; ?>" />
		<?php } ?>

		<?php foreach ($languages as $language) { ?>
		<input type="hidden" name="config_langdata[<?php echo $language['language_id']; ?>][comment]" value="<?php echo $weight_class['weight_class_id']; ?>" />
		<?php } ?>
		
		<?php foreach ($customer_groups as $customer_group) { ?>
		<input type="hidden" name="config_customer_group_id" value="<?php echo $customer_group['customer_group_id']; ?>" />
		<?php } ?>

		<?php foreach ($customer_groups as $customer_group) { ?>
		<?php if (in_array($customer_group['customer_group_id'], $config_customer_group_display)) { ?>
		<input type="hidden" name="config_customer_group_display[]" value="<?php echo $customer_group['customer_group_id']; ?>" />
		<?php } ?>
		<?php } ?>

		<?php foreach ($informations as $information) { ?>
		<input type="hidden" name="config_account_id" value="<?php echo $information['information_id']; ?>" />
		<?php } ?>        

		<?php foreach ($informations as $information) { ?>
		<input type="hidden" name="config_checkout_id" value="<?php echo $information['information_id']; ?>" />
		<?php } ?>  

		<?php foreach ($order_statuses as $order_status) { ?>
		<input type="hidden" name="config_order_status_id" value="<?php echo $order_status['order_status_id']; ?>" />
		<?php } ?>
								
		<?php foreach ($order_statuses as $order_status) { ?>
		<?php if (in_array($order_status['order_status_id'], $config_processing_status)) { ?>
		<input type="hidden" name="config_processing_status[]" value="<?php echo $order_status['order_status_id']; ?>" />
		<?php } else { ?>
		<input type="hidden" name="config_processing_status[]" value="<?php echo $order_status['order_status_id']; ?>" />
		<?php } ?>
		<?php } ?>
		...
		<input type="hidden" name="config_complete_status[]" value="<?php echo $order_status['order_status_id']; ?>" />
		<?php } else { ?>
		<input type="hidden" name="config_complete_status[]" value="<?php echo $order_status['order_status_id']; ?>" />
		<?php } ?>
		<?php } ?>
		...
		<?php foreach ($order_statuses as $order_status) { ?>
		<input type="hidden" name="config_fraud_status_id" value="<?php echo $order_status['order_status_id']; ?>" />
		<?php } ?>

		<?php foreach ($apis as $api) { ?>
		<input type="hidden" name="config_api_id"value="<?php echo $api['api_id']; ?>" />
		<?php } ?>
			  
		<?php foreach ($informations as $information) { ?>
		<input type="hidden" name="config_affiliate_id" value="<?php echo $information['information_id']; ?>" />	
		<?php } ?>

		<?php foreach ($informations as $information) { ?>
		<input type="hidden" name="config_return_id" value="<?php echo $information['information_id']; ?>" />	 
		<?php } ?>		  

		<?php foreach ($return_statuses as $return_status) { ?>
		<input type="hidden" name="config_return_status_id" value="<?php echo $return_status['return_status_id']; ?>" />
		<?php } ?>
            ...
            <div class="tab-pane active" id="tab-general">
                ...
                <div class="tab-content">
                  ...
                  <div class="tab-pane" id="content-language<?php echo $language['language_id']; ?>">
					...
					<div class="form-group">
                      <label class="col-sm-2 control-label" for="input-meta-slogan<?php echo $language['language_id']; ?>"><?php echo $entry_slogan_srub; ?></label>
                      <div class="col-sm-10">
                        <textarea name="config_langdata[<?php echo $language['language_id']; ?>][slogan_srub]" rows="5" placeholder="<?php echo $entry_slogan_srub; ?>" id="input-meta-slogan<?php echo $language['language_id']; ?>" class="form-control"><?php echo isset($config_langdata[$language['language_id']]) ? $config_langdata[$language['language_id']]['slogan_srub'] : ''; ?></textarea>
                      </div>
                    </div>
                    <div class="form-group">
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-10">
                    <select name="config_banner_id" id="input-layout" class="form-control">
                      <?php foreach ($banner_head as $banner) { ?>
            ...
            <div class="tab-pane" id="tab-store">
                ...
                <div class="tab-content">
                  ...
                  <div class="tab-pane" id="store-language<?php echo $language['language_id']; ?>">
					...
					<div class="form-group">
                      <label class="col-sm-2 control-label" for="input-description<?php echo $language['language_id']; ?>"><span data-toggle="tooltip" data-container="#tab-general" title="<?php echo $help_contact_page; ?>">Информация на странице контакты</span></label>
                      <div class="col-sm-10">
                        <textarea name="config_langdata[<?php echo $language['language_id']; ?>][contact_page_inf]" rows="5" placeholder="Информация на странице контакты" id="input-description<?php echo $language['language_id']; ?>" class="form-control"><?php echo isset($config_langdata[$language['language_id']]) ? $config_langdata[$language['language_id']]['contact_page_inf'] : ''; ?></textarea>
                      </div>
                    </div>
                  </div><!-- </div class="tab-pane" id="store-language<?php echo $language['language_id']; ?>">  -->
                  <?php } ?>
